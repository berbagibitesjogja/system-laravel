...
@section('container')
    ...
    <x-table>
        <x-slot:head>
            <x-th>Nama Penerima</x-th>
            <x-th class="hidden sm:table-cell">Jenis</x-th>
        ...
        <x-slot:body>
            @foreach ($beneficiaries as $item)
                <x-tr>
                    <x-td>
                        <div>
                            ...
                            <p class="sm:hidden text-sm text-navy-400">
                                {{ $item->heroes->sum('quantity') }} Orang • {{ round($item->foods() / 1000) }} Kg
                            </p>
                        </div>
                    </x-td>
                    ...
                    <x-td class="hidden sm:table-cell">
                        <span class="font-semibold text-lime-600">{{ round($item->foods() / 1000) }}</span>
                        <span class="text-navy-400">Kg</span>
                    </x-td>
                    @auth
                        <x-td>
                            <div class="flex justify-center gap-2">
                                <x-btn-link href="{{ route('beneficiary.show', $item->id) }}" variant="tosca" class="!p-2">
                                ...
                                <x-btn-link href="{{ route('beneficiary.edit', $item->id) }}" variant="yellow" class="hidden md:inline-flex !p-2">
                                ...
                                @if (auth()->user()->role == 'super')
                                    <form action="{{ route('beneficiary.destroy', $item->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
