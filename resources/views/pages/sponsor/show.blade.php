...
@section('container')
    ...
    <div class="bg-white rounded-2xl border border-navy-100 shadow-md p-6 mb-8">
        <h1 class="text-2xl font-bold text-navy-900 mb-2">{{ $sponsor->name }}</h1>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
            <div class="bg-gradient-to-br from-white to-tosca-50 p-4 rounded-xl border border-navy-100">
                <p class="text-sm text-navy-400">Penerima</p>
                <p class="text-2xl font-bold text-tosca-600">{{ $sponsor->heroes->sum('quantity') }} <span class="text-sm text-navy-400">Orang</span></p>
            </div>
            <div class="bg-gradient-to-br from-white to-lime-50 p-4 rounded-xl border border-navy-100">
                <p class="text-sm text-navy-400">Makanan</p>
                <p class="text-2xl font-bold text-lime-600">{{ round($sponsor->foods->sum('weight') / 1000) }} <span class="text-sm text-navy-400">kg</span></p>
            </div>
            <div class="bg-gradient-to-br from-white to-orange-50 p-4 rounded-xl border border-navy-100">
                <p class="text-sm text-navy-400">Total Aksi</p>
                <p class="text-2xl font-bold text-orange-500">{{ $sponsor->donation->count() }} <span class="text-sm text-navy-400">Aksi</span></p>
            </div>
        </div>
    </div>

    <h2 class="text-xl font-bold text-navy-900 mb-4">Donasi Oleh {{ $sponsor->name }}</h2>
